				...
				if(accepterror == 0)
				{
					...
					else if(pid == 0)
					{
						...
						*/
						...
						/*itoa(client.avg_time,val,10);
						//qstrcpy(val,client.avg_time);
						strcat(val," ");
						strcat(val,client.name);*/
						fscanf(fptr,"%s",num);
						fscanf(fptr,"%s",c);
						fclose(fptr);
						min = atoi(num);
						...
						{
							fopen("LOG.txt","w");
							sprintf(val,"%d",Avg_time);
							//itoa(Avg_time,val,10);
							printf("THE MINIMUM TIME IS %d BY THE USER %s\n",Avg_time,client.name);
							
							fprintf(fptr, "%s", val);
							fprintf(fptr, "%s", " ");
							fprintf(fptr, "%s", client.name);
							fclose(fptr);
						}
						

						//CD[numclient].avg = client.avg_time;
						printf("\nOVER\n");
						over[0] = '1';
						over[1] = '\0';
						send(s_server_child, over, sizeof(over), 0);
						send(s_server_child, winner, sizeof(winner), 0);
						char result[25];
